...
> 已验证后端：synthetic；海康 `MV-CS050-60GM` 的 MVS 适配器已实现但仍需现场 SDK/设备验收
> 大恒相机：当前 `main` 尚未提供 `daheng` backend，适配工作使用 `feature/daheng-camera-support` 分支
...
| `python -m pytest -q` | 77 通过，1 失败 | 失败项为历史 golden snapshot 漂移检查 |
...
接入海康真机时，把配置换成 `camera.mvs.example.yaml`，确认 MVS SDK、设备型号和序列号后再运行 `camera-list`。若 MVS 客户端正在取流，应先在客户端停止并断开相机，再运行标定向导。大恒相机在 `main` 尚未支持，不能把 `backend: mvs` 配置直接用于大恒设备。
...
当前快照为 77 个测试通过、1 个 golden snapshot 失败；失败来自历史 baseline 漂移，而不是 CLI 导入失败。
...
当前 `main` 只接受 `mvs` 和 `synthetic` 两种 backend，不能用海康 MVS 配置直接连接大恒相机。大恒适配使用分支 `feature/daheng-camera-support`，目标是新增独立的 Daheng SDK adapter 和 `camera.daheng.example.yaml`，并复用现有 `CameraProvider`/`CameraSession` 接口。

在没有大恒 SDK 或设备的环境中，先完成 fake provider/synthetic 单元测试：设备枚举、型号/序列号、曝光和增益的实际回读、PixelFormat/ROI 停流重配、帧号与相机时间戳、超时、断流、`stop()`/`close()` 幂等性。接入真实设备后，还要现场确认 GigE 网络、SDK 与 Python 位数、触发模式、Mono8/Mono12 数据类型、相机回读值和连续取流稳定性。适配完成前，请继续使用 synthetic 或海康 MVS 后端，不要在项目配置中填写未实现的 `backend: daheng`。
...
6. 点击“开始采集”；正式三联图每个姿态按 chess→nolaser→laser 完成后才移动到下一姿态。

当前 PySide6 向导的推荐完整操作顺序是：先在第 1 页应用项目并在第 2 页确认相机；在第 3 页填写数据集 ID、输出目录和计划 YAML 路径，点击“生成并检查计划”后计划会自动保存到数据集目录之外（例如 `projects/default/plans/laser_plane.yaml`），并立即用 `load_capture_plan()` 做 round-trip 校验。默认三联图写入 `projects/default/data/laser_plane/fit` 和 `projects/default/data/laser_plane/validation`，文件名为 `chess 001.tif`、`nolaser 001.tif`、`laser 001.tif`。默认 `nolaser`/`laser` 曝光为 500 μs。

点击“开始采集”后，页面会自动切换到第一个未完成任务并启动持续实时预览。点击“稳定后保存当前帧”时，工具会在当前任务曝光下丢弃 `settle_frames` 帧、保存下一帧，然后在线应用下一个任务的曝光/增益和质量模式，实时画面管道保持连接；只有 ROI 或像素格式等结构性参数改变时才会执行必要的停流重配。三联图完成一组后再移动棋盘。页面中的“取消”会停止预览并保留 `.inprogress` 数据集；下次保持计划不变，勾选“续采”即可继续，续采仍会校验原计划 hash。采集成功后页面会记录 plan、数据集根目录、fit/validation、`dataset_manifest.yaml` 和 `frames.csv`，并写入项目 YAML 的 `extra.capture_artifacts`（旧项目字段仍兼容）。

按钮分工：上排“预览选中任务”用于在正式采集前试看所选任务的曝光和实时画面；“稳定后保存当前帧”是唯一的保存入口，保存完成后自动切换下一任务；“下一任务”只用于手动改变预览选择。下排“开始采集”启动逐任务实时引导，“续采”从 `.inprogress` 的首个未完成任务启动，“取消”停止当前引导并释放相机。不再另设重复的“采集当前任务”按钮。
...
nolaser 001.tif  低曝光、激光关闭，仅用作背景扣除
laser 001.tif    相同低曝光、激光开启，用于激光中心和可用时的运动匹配
```
...
- 点击“稳定后保存当前帧”时，还会在当前任务参数下再次丢弃 `settle_frames` 帧，再保存下一帧，因此不会把曝光切换过程中的旧帧写入数据集；
...
同时把默认提取算法切到与标定链一致的实时 `steger`，并让它读取中央 profile。这样以后只修改 `calibration/config/realtime_steger.yaml` 的 `sigma` 等参数，标定和在线测量会一起更新：
...
| `pair_motion` 报 `pair_movement` | 三联图之间检测到可测量位移（>1 px） | 固定支架，按 chess→nolaser→laser 顺序重新采集该组 |
